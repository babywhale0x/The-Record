...
export default function WalletModal() {
  ...
  // Deduplicate Aptos Connect wallets — only show one
  const uniqueAptosConnect = aptosConnectWallets.slice(0, 1)
  ...
  const handleConnect = async (walletName: string) => {
    try {
      ...
      // If it tries to open petra.app/web — intercept and open correct URL
      if (msg.includes('petra.app/web') || msg.includes('aptos-connect-is-now-petra-web')) {
  ...
  const handleAptosConnect = async (walletName: string) => {
    // Aptos Connect (Google/Apple) requires the aptosconnect.app flow
    // The wallet adapter redirects to a broken URL, so we open directly
    window.open('https://aptosconnect.app', '_blank')
  }

  const handleAptosConnectGoogle = () => {
    window.open('https://aptosconnect.app', '_blank')
  }

  const handleAptosConnectApple = () => {
    window.open('https://aptosconnect.app', '_blank')
  }
  ...
  return (
    <div className={styles.overlay} onClick={close}>
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        {/* Aptos Connect — social login — deduplicated, correct URL */}
        {uniqueAptosConnect.length > 0 && (
          <div className={styles.section}>
            <span className={styles.sectionLabel}>SIGN IN — NO EXTENSION NEEDED</span>
            <div className={styles.socialRow}>
              <button
                className={styles.socialBtn}
                onClick={handleAptosConnectGoogle}
              >
              ...
              <button
                className={styles.socialBtn}
                onClick={handleAptosConnectApple}
              >
            ...
            <p className={styles.socialNote}>Powered by Aptos Connect · No extension needed</p>
          </div>
        )}

        <div className={styles.divider}><span>or use a wallet extension</span></div>

        {/* Installed extension wallets */}
